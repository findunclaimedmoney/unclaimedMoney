...
async function sendFoundEmail(opts: {
  ...
  if (!stripeKey || !resendKey) {
    logger.warn(
      ...
      "auto-search: skipping found-email — missing keys",
    );
    return;
  }
  ...
  try {
    ...
    logger.error(
      { err, searchId },
      "auto-search: failed to create Stripe session",
    );
    return;
  }

  const matchRows = teaserMatches
    .slice(0, 3)
    .map((m) => {
...
<div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#061826;padding:0;border-radius:12px;overflow:hidden;">
  ...
  try {
    ...
    logger.info(
      { searchId, email, totalAmountCents },
      "auto-search: found-email sent",
    );
  } catch (err) {
    logger.error(
      { err, searchId, email },
      "auto-search: failed to send found-email",
    );
  }
}
  ...
  try {
    ...
    if (todayCount >= DAILY_LIMIT) {
      logger.info(
        ...
        "auto-search: daily limit reached, skipping",
      );
      return;
    }
    ...
    logger.info(
      { lead: `${lead.firstName} ${lead.lastName}`, source: lead.sourceTable },
      "auto-search: starting search",
    );
    ...
    const results = await searchAllSources({
      ...
      dob: lead.dob !== "unknown" ? lead.dob : undefined,
    });
    const validMatches = results.matches.filter(
      (m) => m.name && m.holder !== undefined,
    );
    ...
    logger.info(
      {
        ...
        totalAmountCents,
        source: lead.sourceTable,
      },
      "auto-search: search complete",
    );

    if (hasMatches) {
      await sendFoundEmail({
        searchId,
        email: lead.email,
        firstName: lead.firstName,
        lastName: lead.lastName,
...
const REPORT_EMAIL =
  process.env.ADMIN_REPORT_EMAIL ?? "admin@missingcash.com.au";
...
async function sendDailyReport() {
  ...
  const totalFoundCents = hits.reduce(
  ...
  const hitRows = hits
    .map((r) => {
      ...
      const outreach = OUTREACH_TEMPLATE.replace(/\[NAME\]/g, r.firstName)
...
<div style="font-family:sans-serif;max-width:700px;margin:0 auto;background:#061826;padding:0;border-radius:12px;overflow:hidden;">
  ...
  try {
    ...
    logger.info(
      { hits: hits.length, totalSearches, totalFoundCents },
      "daily-report: sent",
    );
  } catch (err) {
...
export function startAutoSearch() {
  logger.info(
    { intervalMs: INTERVAL_MS, dailyLimit: DAILY_LIMIT },
    "auto-search: scheduler started",
  );
  void runAutoSearch();
  setInterval(() => {
    void runAutoSearch();
